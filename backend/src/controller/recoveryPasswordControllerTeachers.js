...
const recoveryPasswordControllerTeachers = {}

recoveryPasswordControllerTeachers.requestCode = async (req, res) => {
    try {
        const { email } = req.body;
        const userFound = await teachers.findOne({ email });
        if(!userFound){
            return res.status(404).json({message: "user not found"})
        }
        const randomCode = crypto.randomBytes(3).toString("hex")
        const token = jsonwebtoken.sign(
            {email, randomCode, userType: "teachers", verified: false},
            config.JWT.secret,
            {expiresIn: "15m"},
        );
...
recoveryPasswordControllerTeachers.verifyCode = async (req, res ) => {
    try {
        const { code } = req.body;
...
recoveryPasswordControllerTeachers.newPassword = async (req, res) => {
...
export default recoveryPasswordControllerTeachers;